<template>
    <div>
         ...
         <div class=" max-w-5xl shadow-md m-auto min-h-screen pt-3">
            <div class=" px-2">
              <div class="topAnime">
                <div class=" h-10 w-full bg-green-900">
                    <h1 class="text-2xl px4 text-white font-bold">최신 애니:</h1>
                </div>
            ...
            <div class=" px-2">
               <div class=" h-10 w-full bg-green-900">
                 <h1 class="text-2x1 px-4 text-white font-bold">Anime Movie</h1>
               </div>
               <div class="animeMovie flex flex-wrap justify-center">
                 <Card v-for="anime in response" :key="anime.mal_id" class="mx-auto" @click="goAnime(anime)">
                  <template #image>
                             <img :src="anime.images.webp.image_url" height="266" alt="anime.title"/>
                         </template>
...
async function getData() {
        try {
            ...
            if(data.value) {
             ...
             const { data: refs } = await useLazyFetch(`https://api.jikan.moe/v4/top/anime?type=movie`);
             if(result.value) {
                 Movie.value = result.value.data
             }
        } catch(error) {
            console.log(error)
        }
